import pytest

from src.generators import get_data, filter_by_currency, transactions, card_number_generator, transaction_descriptions


@pytest.fixture
def data() -> list[dict]:
    return [
        {
            "id": 939719570,
            "state": "EXECUTED",
            "date": "2018-06-30T02:08:58.425572",
            "operationAmount": {
                "amount": "9824.07",
                "currency": {
                    "name": "USD",
                    "code": "USD"
                }
            },
            "description": "Перевод организации",
            "from": "Счет 75106830613657916952",
            "to": "Счет 11776614605963066702"
        },
        {
            "id": 142264268,
            "state": "EXECUTED",
            "date": "2019-04-04T23:20:05.206878",
            "operationAmount": {
                "amount": "79114.93",
                "currency": {
                    "name": "USD",
                    "code": "USD"
                }
            },
            "description": "Перевод со счета на счет",
            "from": "Счет 19708645243227258542",
            "to": "Счет 75651667383060284188"
        },
        {
            "id": 873106923,
            "state": "EXECUTED",
            "date": "2019-03-23T01:09:46.296404",
            "operationAmount": {
                "amount": "43318.34",
                "currency": {
                    "name": "руб.",
                    "code": "RUB"
                }
            },
            "description": "Перевод со счета на счет",
            "from": "Счет 44812258784861134719",
            "to": "Счет 74489636417521191160"
        },
        {
            "id": 895315941,
            "state": "EXECUTED",
            "date": "2018-08-19T04:27:37.904916",
            "operationAmount": {
                "amount": "56883.54",
                "currency": {
                    "name": "USD",
                    "code": "USD"
                }
            },
            "description": "Перевод с карты на карту",
            "from": "Visa Classic 6831982476737658",
            "to": "Visa Platinum 8990922113665229"
        },
        {
            "id": 594226727,
            "state": "CANCELED",
            "date": "2018-09-12T21:27:25.241689",
            "operationAmount": {
                "amount": "67314.70",
                "currency": {
                    "name": "руб.",
                    "code": "RUB"
                }
            },
            "description": "Перевод организации",
            "from": "Visa Platinum 1246377376343588",
            "to": "Счет 14211924144426031657"
        }
    ]


def test_get_data(data: list[dict]) -> None:
    assert get_data() == data


def test_filter_by_currency() -> None:
    usd_transactions = filter_by_currency(transactions, 'USD')
    rub_transactions = filter_by_currency(transactions, 'RUB')

    assert next(usd_transactions)['id'] == 939719570
    assert next(usd_transactions)['id'] == 142264268
    assert next(rub_transactions)['id'] == 873106923
    assert next(usd_transactions)['id'] == 895315941
    assert next(rub_transactions)['id'] == 594226727


def test_transaction_descriptions() -> None:
    descriptions = transaction_descriptions(transactions)

    assert next(descriptions) == 'Перевод организации'
    assert next(descriptions) == 'Перевод со счета на счет'
    assert next(descriptions) == 'Перевод со счета на счет'
    assert next(descriptions) == 'Перевод с карты на карту'
    assert next(descriptions) == 'Перевод организации'


def test_card_number_generator() -> None:
    card_number = card_number_generator(5, 9)

    assert next(card_number) == '0000 0000 0000 0005'
    assert next(card_number) == '0000 0000 0000 0006'
    assert next(card_number) == '0000 0000 0000 0007'
    assert next(card_number) == '0000 0000 0000 0008'
    assert next(card_number) == '0000 0000 0000 0009'
